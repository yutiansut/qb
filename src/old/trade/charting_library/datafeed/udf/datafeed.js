...
Datafeeds.UDFCompatibleDatafeed.prototype._initialize = function() {
	var that = this;
	// console.log('Datafeeds _initialize 第一次请求')
	this._send(this._datafeedURL + '/config')
		.done(function(response) {
			var configurationData = JSON.parse(response);
			// console.log('Datafeeds _initialize _send done 第一次请求数据回来', configurationData)
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
	...
	// console.log('UDFCompatibleDatafeed getBars 轮循请求数据getBars')
	...
	.done(function(response) {
		...
		// console.log('UDFCompatibleDatafeed getBars 轮循请求数据getBars 回来', data, data.s !== 'ok' && !nodata, !!onErrorCallback)
		...
		// console.log('UDFCompatibleDatafeed getBars 轮循请求数据getBars 回来 onDataCallback', bars, { noData: nodata, nextTime: data.nb || data.nextTime })
...
Datafeeds.DataPulseUpdater = function(datafeed, updateFrequency) {
	...
	var update = function() {
		// console.log('DataPulseUpdater update 轮循请求数据开始')
		...
		for (var listenerGUID in that._subscribers) {
			...
			(function(_subscriptionRecord) { // eslint-disable-line
				// console.log('DataPulseUpdater getBars 轮循请求数据getBars')
				that._datafeed.getBars(_subscriptionRecord.symbolInfo, resolution, datesRangeLeft, datesRangeRight, function(bars) {
					// console.log('DataPulseUpdater getBars 轮循请求数据getBars ondata', bars)
					that._requestsPending--;
