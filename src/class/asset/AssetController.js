...
export default class AssetController extends ExchangeControllerBase {
                 ...
                 filte(wallet, value, hideLittle, hideZero) {
                   let arr1 = this.filter(wallet, item => {
                     return item.coinName.includes(value.toUpperCase()) || item.fullname.includes(value.toUpperCase());
                   });
                 ...
                 appendAddress({ addressName, address }) {
                   ...
                   this.view.state.walletExtract.extract_addr.push(
                     { addressName, address }
                   );
                   this.view.setState({
                     walletExtract: this.view.state
                       .walletExtract
                   });
                 }
                 ...
                 deletAddress({ addressName, address }) {
                   ...
                   this.view.state.walletExtract.extract_addr = this.view.state.walletExtract.extract_addr.filter(item => item.address !== address);
                   this.view.setState({
                     walletExtract: this.view.state
                       .walletExtract
                   });
