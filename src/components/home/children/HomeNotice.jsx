...
export default class homeNotice extends ExchangeViewBase {
  ...
  render() {
    ...
    return <div className={`${this.state.noticeList && Object.keys(this.state.noticeList).length && this.state.noticeList.data.length ? "" : "hide"} home-notice-wrap`}>
      {this.state.noticeList && Object.keys(this.state.noticeList).length && this.state.noticeList.data.length && <div className="home-notice-content">
        <ul style={{top: this.state.top1 + "%"}}>
          {this.state.noticeList && Object.keys(this.state.noticeList).length && this.state.noticeList.data.length && this.state.noticeList.data.map((v, index) => <li key={index}>
