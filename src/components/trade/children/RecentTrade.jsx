                ...
                index <= 50 && (
                  <tr key={index}>
                    ...
                    <td  style={{color: this.state.recentItemSelect !== 'mineLess' ? (v.orderType === 1 ? '#f25656' : '#2bb789') : 'rgba(255,255,255,0.8)'}}>
                      {this.state.recentItemSelect === 'mineLess' ? ((this.state.unitsType === "CNY" && Number(v.price * this.state.recentBank.cny).format({number:'legal',style:{name:'cny'}})) || ((this.state.unitsType === "USD" && Number(v.price * this.state.recentBank.usd).format({number:'legal',style:{name:'usd'}})) || Number(v.price).format({number:'digital', style:{decimalLength :this.props.controller.accuracy.priceAccuracy}}))): ((this.state.unitsType === "CNY" && Number(v.avgPriceCN).format({number:'legal',style:{name:'cny'}})) || ((this.state.unitsType === "USD" && Number(v.avgPriceEN).format({number:'legal',style:{name:'usd'}})) || Number(Number(v.avgPrice).toFixed(this.props.controller.accuracy.priceAccuracy)).format({number:'digital', style:{decimalLength :this.props.controller.accuracy.priceAccuracy}})))}
                    </td>
