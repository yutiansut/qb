...
class ReactKDepth extends exchangeViewBase {
     ...
     */
    setData(data){
        if (this.state.kdepth && data.bids.length && data.asks.length && this.state.kdepth) {
            this.state.kdepth.setData(data);
            return;
        }
        this.state.kdepth.setData({ asks: [[0, 0],[0, 0]], bids: [[0, 0],[0, 0]]});
    ...
    setLanguage(lang){
        this.state.kdepth.setLanguage(lang);
    }
